...
class FleetSizeOptimizer:
    def __init__(self, flight_time, energy_consumption, f_values, fixed_cost=1, variable_cost=0.0000001, time_step=5):
        ...
        self.soc_transition_time = np.array([ 3.246,  3.744,  4.422,  5.412,  6.966,  9.78 , 16.596, 87.708])
                ...
                for j in V:
                    for t in range(T):
                        if i != j:
                            ...
                            m.addConstr(sij[t, i, j] >= self.f_values[t, i, j] - uij.sum(t, i, j, '*'))
    ...
    def parse_result(self, output_file):
        results = pd.read_table(f'{output_file}')
